...
describe('DLCManager', () => {
    ...
    describe('createDLC', () => {
        it('emits an event with correct data', async () => {
            const transaction = await dlcManager.createDLC(
                emergencyRefundTime,
                0
            );
            const txReceipt = await transaction.wait();
            const event = txReceipt.events.find(
                (ev) => ev.event == 'CreateDLC'
            );

            expect(event.event).to.equal('CreateDLC');
            expect(event.args.uuid).to.not.equal(undefined);
            expect(event.args.creator).to.equal(
                '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266'
            );
            expect(event.args.emergencyRefundTime.toNumber()).to.equal(
                emergencyRefundTime
            );
    ...
    describe('postCreateDLC', async () => {
        ...
        beforeEach(async () => {
            const setupLoanTx = await lendingContract
                .connect(user)
                .setupLoan(1, 14000, 1000, emergencyRefundTime);
            const txReceipt = await setupLoanTx.wait();

            const setupLoanEvent = txReceipt.events.find(
                (event) => event.event == 'SetupLoan'
            );
        ...
        it('emits an event with correct data', async () => {
            ...
            const event = txReceipt2.events.find(
                (event) => event.event == 'PostCreateDLC'
            );

            expect(
                Object.keys(event.args).filter((key) => isNaN(key)).length
            ).to.equal(6); // for some reason the event args appear twice, once with a key and once without.
            ...
            expect(event.args.emergencyRefundTime).to.equal(
                emergencyRefundTime
            );
        ...
        it('calls back into the provided protocol contract', async () => {
            ...
            let loan = await lendingContract.getLoan(nonce);
            ...
            // Ready after tx
            loan = await lendingContract.getLoan(nonce);
    ...
    describe('setStatusFunded', async () => {
        ...
        it('emits an event with correct data', async () => {
            ...
            const event = txReceipt.events.find(
                (event) => event.event == 'SetStatusFunded'
            );

            expect(event.args.uuid).to.equal(uuid);
            expect(event.args.eventSource).to.equal(
                'dlclink:set-status-funded:v0'
            );
        });
    });
});
